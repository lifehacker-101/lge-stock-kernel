...
static const char defaultFirmware[] = "atmel/t1lte_global_com/t1_0405v28.fw";
#elif defined(CONFIG_MACH_MSM8974_T1WIFI_GLOBAL_COM)
static const char defaultFirmware[] = "atmel/t1wifi_global_com/t1_0405v28.fw";
#elif defined(CONFIG_MACH_MSM8974_T1WIFIN_GLOBAL_COM)
static const char defaultFirmware[] = "atmel/t1wifin_global_com/t1_0405v28.fw";
#endif
...
static ssize_t mxt_run_delta_show(struct i2c_client *client, char *buf)
{
	...
	if (ts->minios)
		write_file(DELTA_FILE_PATH_MINIOS, buf, 1);
	else
		write_file(DELTA_FILE_PATH, buf, 1);
	msleep(30);
	run_delta_read(ts, ref_buf, &len);
	if (ts->minios)
		write_file(DELTA_FILE_PATH_MINIOS, ref_buf, 0);
	else
...
static int mXT2954_DoSelfDiagnosis(struct i2c_client *client, int* pRawStatus, int* pChannelStatus, char* pBuf, int bufSize, int* pDataLen)
{
	...
	if (ts->minios)
		write_file(SELF_DIAGNOSTIC_FILE_PATH_MINIOS, pBuf, 1);
	else
	...
	if (ts->minios)
		write_file(SELF_DIAGNOSTIC_FILE_PATH_MINIOS, pBuf, 0);
	else
