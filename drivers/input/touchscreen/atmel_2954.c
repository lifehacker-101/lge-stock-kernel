...
static void mxt_probe_regulators(struct mxt_data *data)
{
	...
	if (IS_ERR(data->vdd_ana)) {
		...
		dev_err(dev, "Error %d getting unuse ana regulator\n", error);
		goto fail;
	}
	...
	if (error < 0) {
		dev_err(dev, "Error %d cannot control unuse ana regulator\n", error);
		goto fail;
	}
	...
	error = regulator_enable(data->vdd_ana);
	if (error < 0) {
		dev_err(&data->client->dev, "unuse vdd_ana regulator enable fail\n");
		goto fail;
	}
	...
	error = regulator_disable(data->vdd_ana);
	if (error < 0) {
		dev_err(&data->client->dev, "unuse vdd_ana regulator disable fail\n");
		goto fail;
	}

	TOUCH_INFO_MSG("ldo22 power status : %d\n", regulator_is_enabled(data->vdd_ana));
...
static LGE_TOUCH_ATTR(fw_ver, S_IRUGO | S_IWUSR, mxt_testmode_ver_show, NULL);
...
static void mxt_gesture_mode_start(struct mxt_data *data)
{
	...
	if (global_mxt_data) {
		...
		reg = data->T72_address + 0 ;
		TOUCH_INFO_MSG("slimport is connected. T72 enable.\n");
		mxt_write_reg(data->client, reg, 11);
	}
#else
		if (mxt_patchevent_get(PATCH_EVENT_TA)) {
			mxt_patch_event(global_mxt_data, CHARGER_KNOCKON_SLEEP);
...
static int mxt_parse_dt(struct device *dev, struct mxt_platform_data *pdata)
{
	...
	else
	{
		pdata->gpio_ldo1= -1;
		pdata->gpio_ldo2= -1;
		TOUCH_INFO_MSG("unuse externel LDO control. HW rev = %d \n",pdata->hw_rev);
	}
...
int fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
       ...
       if (evdata && evdata->data && event == FB_EVENT_BLANK && ts && ts->client) {
               ...
               if (*blank == FB_BLANK_UNBLANK){
                       ...
                       fb_blank_called = 0;
                       if(lge_get_boot_mode() != LGE_BOOT_MODE_NORMAL) { // not-normal mode
                              mxt_fb_resume(ts);
                       }
                       else{
