...
void rt8555_backlight_on(int level)
{
	...
	if (factory_off_flag) {
		...
		/* mixed, i2c, spread edge rate control normal, w/o Spread Spectrum, mix-26k */
		rt8555_write_reg(main_rt8555_dev->client, 0x00, 0xA3);
		/* 8bit, ovp 24V, current limit 2.5A, Freq. 1Mhz */
		rt8555_write_reg(main_rt8555_dev->client, 0x01, 0x17);
		/* LED Current 20.04mA => 18.26mA */
		rt8555_write_reg(main_rt8555_dev->client, 0x02, 0x8B);
		mdelay(80);
	}
...
#if defined(CONFIG_MACH_MSM8974_T1WIFIN_GLOBAL_COM)
	msleep(50);
	rt8555_set_main_current_level(main_rt8555_dev->client, 0);
#endif
...
static int bl_set_intensity(struct backlight_device *bd)
{
	...
	rt8555_lcd_backlight_set_level(bd->props.brightness);
#else
